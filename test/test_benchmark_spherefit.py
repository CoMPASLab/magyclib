"""
Test Module for the Benchmark Sphere Fit Module
...
from magyc.benchmark_methods import sphere_fit
...
class TestSphereFit(unittest.TestCase):
    def test_input_type_and_shape(self):
        ...
        hard_iron, calibrated_magnetic_field = sphere_fit(magnetic_field_list)
        ...
        hard_iron, calibrated_magnetic_field = sphere_fit(magnetic_field_array)
        self.assertEqual(hard_iron.shape, (3,))
        self.assertEqual(calibrated_magnetic_field.shape, TEST_MAGNETIC_FIELD.shape)

        # Test with a transposed numpy array input
        magnetic_field_array_T = magnetic_field_array.T
        hard_iron, calibrated_magnetic_field = sphere_fit(magnetic_field_array_T)
        self.assertEqual(hard_iron.shape, (3,))
        self.assertEqual(calibrated_magnetic_field.shape, TEST_MAGNETIC_FIELD.shape)
